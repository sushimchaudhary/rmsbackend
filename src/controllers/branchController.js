...
/* ===================== CREATE BRANCH auto assign subscription ===================== */
...
exports.createBranch = async (req, res) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Branch create मात्र गर्ने (Subscription Auto-Assign हटाइएको छ)
      const branch = await tx.branch.create({
        data: {
          name: name.trim(),
          address: address.trim(),
    ...
    res.status(201).json({
      data: result,
      message: "Branch created successfully. Please select a subscription plan to activate.",
    });
  } catch (err) {
    console.error("CREATE BRANCH ERROR:", err);
    res.status(400).json({ response: err.message || "Server error" });
