...
const processBodyAndFiles = async (req, resource, defaultImageField = 'image') => {
  ...
  // 1. Prisma Relation Field delete गर्नुहोस् (Extra payload parameters cleaning)
  delete data.branch;
  ...
  // 🟢 8. Organizational Detail Auto-Fill (Restaurant & Branch Data)
  // if (resource === 'organization' && data.branch_id) {
  ...
  //   if (branchInfo) {
  //     // यदि pan_vat_number पठाइएको छैन भने Restaurant को PAN/VAT Auto Fill गर्ने
  //     if (!data.pan_vat_number) {
  //       data.pan_vat_number = branchInfo.restaurant?.pan_number || branchInfo.restaurant?.vat_number || null;
  //     }
  //     // यदि title/name खाली छ भने Restaurant/Branch को नाम Auto Fill गर्ने
  //     if (!data.title) {
  //       data.title = `${branchInfo.restaurant?.name || ''} - ${branchInfo.name}`;
  //     }
  //     // यदि ठेगाना खाली छ भने Branch को ठेगाना Auto Fill गर्ने
  //     if (!data.address) {
  //       data.address = branchInfo.address;
  //     }
  //   }
  // }
...
if (resource === 'organization' && data.branch_id) {
  ...
  if (branchInfo) {
    // 🟢 यदि pan_vat_number खाली/undefined आएको छ भने Auto Fill गर्ने:
    if (!data.pan_vat_number || data.pan_vat_number.trim() === "") {
      const p = branchInfo.restaurant?.pan_number;
      const v = branchInfo.restaurant?.vat_number;
      
      data.pan_vat_number = (p && v) ? `${p} / ${v}` : (p || v || null);
    }
    ...
    if (!data.address || data.address.trim() === "") {
      data.address = branchInfo.address || branchInfo.restaurant?.address || '';
    }
  }
}
